...
/************************************************************
  ...
  function parseWarrantSummaryFields(e) {const raw = norm(String(e?.warrantSummary || ''));
                                         ...
                                         const eventDateMatch = raw.match(/(?:^|\n)\s*(\d{1,2}\/\d{1,2}\/\d{4})\s+Event:\s*/i) || eventRaw.match(/^(\d{1,2}\/\d{1,2}\/\d{4})\b/);
                                         const eventBondMatch = raw.match(/(?:^|\n)\s*(?:\d{1,2}\/\d{1,2}\/\d{4}\s+)?Event:\s*[^\n]*?\bBond Amount:\s*([0-9][\d,]*(?:\.\d{2})?)/i)
                                                             || eventRaw.match(/\bBond Amount:\s*([0-9][\d,]*(?:\.\d{2})?)/i);
