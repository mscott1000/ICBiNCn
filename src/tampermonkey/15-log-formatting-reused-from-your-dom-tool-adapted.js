...
SYCAMORE HILLS (OPERATES IN ST. JOHN MUNICIPAL) - (314) 427-8700 EXT. 6`;
  ...
  function getMunicipalityHeaderForSummary(jurisdiction,judgeName = '') {const key = municipalityKey(jurisdiction);
                                                                        ...
                                                                        if (matchKey === 'CITY OF ST LOUIS') return 'CITY OF ST. LOUIS CIRCUIT - (314) 622-4500';
  ...
  function buildSummaryCopyText() {const log = loadLog();
                                  ...
                                  const upcomingLines = [];
                                  if (hasUpcoming) {upcomingLines.push('Upcoming Court Dates:', '');
                                                   for (const [jurisdiction,dateMap] of upcomingByJurisdiction.entries()) {upcomingLines.push(jurisdiction);
                                                                                                                              for (const [dt,caseNos] of dateMap.entries()) {upcomingLines.push(`${dt} (for ${formatCaseNumberList(caseNos)})`);}
                                                                                                                              upcomingLines.push('');}}
                                  const upcomingText = upcomingLines.join('\n').trim();
                                  const ineligibleBlock = [ineligibleText,upcomingText].filter(Boolean).join('\n\n');
