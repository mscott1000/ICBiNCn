/************************************************************
  ...
  dock.addEventListener('click',async (e) => {const id = e?.target?.id;
                                             ...
                                             if (id === 'moUpcomingCopyDebug') {GM_setClipboard(upcomingStorageGet(UPCOMING_DEBUG_KEY),'text'); upcomingSetStatus('Upcoming Court Dates debug copied to clipboard.'); return;}
                                             if (id === 'moUpcomingClearDebug') {upcomingStorageSet(UPCOMING_DEBUG_KEY,''); const debug = document.getElementById('moUpcomingDebug'); if (debug) debug.value = ''; upcomingSetStatus('Upcoming Court Dates debug cleared.'); return;}
  ...
  dock.addEventListener('click',(e) => {const action = e?.target?.dataset?.tbAction;
                                     ...
                                     if (action === 'confirmIneligible') {if (textBuilderState.data.workflow === 'walkInDropIn' || ['Can’tDo: Out of Network','Can’tDo: Repeat Attempt','Can’tDo: Fresh Start Friday'].includes(textBuilderState.data.researchType)) return textBuilderSet('message'); if (textBuilderState.data.researchType === 'CanDo: + Can’tDo') return textBuilderSet('address'); return textBuilderSet('courts',{courts:[]});}
