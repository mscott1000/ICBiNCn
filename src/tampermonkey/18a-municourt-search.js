/************************************************************
  ...
  const MUNI_REMOTE_CLOSE_FALLBACK_MS = 15000;
  const MUNI_REMOTE_HASH_PREFIX = '#icbincn-muni-task=';

  function isMuniNameSearchPage() {try {const u = new URL(location.href);
                                        return /municourt\.net$/i.test(u.hostname || '') && u.pathname === '/' && u.searchParams.get('sel') === '0';}
                                 catch {return false;}}

  function getMuniRemoteTaskIdFromUrl() {if (!isMuniNameSearchPage()) return '';
                                         try {const hash = String(location.hash || '');
                                              if (!hash.startsWith(MUNI_REMOTE_HASH_PREFIX)) return '';
                                              return decodeURIComponent(hash.slice(MUNI_REMOTE_HASH_PREFIX.length));}
                                         catch {return '';}}

  function getMuniRemoteOpenUrl(taskId) {return `${MUNI_NAME_SEARCH_URL}${MUNI_REMOTE_HASH_PREFIX}${encodeURIComponent(String(taskId || ''))}`;}

  function closeMuniRemoteTab(reason = 'complete') {if (!getMuniRemoteTaskIdFromUrl()) return;
                                                    dbg('municourt_remote_tab_close',{reason,url: location.href});
                                                    try {window.close();}
                                                    catch {}}

  function scheduleMuniRemoteTabClose(reason,delayMs = 0) {if (!getMuniRemoteTaskIdFromUrl()) return;
                                                          setTimeout(() => closeMuniRemoteTab(reason),Math.max(0,Number(delayMs || 0)));}
  ...
  async function runMunicourtRemoteWorkerIfNeeded() {if (!/municourt\.net$/i.test(location.hostname || '')) return;
                                                     const remoteTaskId = getMuniRemoteTaskIdFromUrl();
                                                     if (!remoteTaskId) return;
                                                     const task = await gmGetValueSafe(MUNI_REMOTE_TASK_KEY,null);
                                                     if (!task?.id || task?.done) {scheduleMuniRemoteTabClose('no_active_task_fallback',MUNI_REMOTE_CLOSE_FALLBACK_MS);
                                                                                  return;}
                                                     if (remoteTaskId !== task.id) return;
                                                     const now = Date.now();
                                                     const createdAt = Number(task?.createdAt || 0);
                                                     if (createdAt && now - createdAt > 120000) {scheduleMuniRemoteTabClose('stale_task_fallback',MUNI_REMOTE_CLOSE_FALLBACK_MS);
                                                                                                return;}
                                                     if (task?.claimedAt && now - Number(task.claimedAt || 0) < 30000) {scheduleMuniRemoteTabClose('claimed_task_fallback',MUNI_REMOTE_CLOSE_FALLBACK_MS);
                                                                                                                        return;}
                                                     ...
                                                     try {const candidates = await searchMuniViaSubmitByName(task?.params || {},{allowBrowserRecaptcha: true});
                                                          ...
                                                          scheduleMuniRemoteTabClose('completed');}
                                                                 ...
                                                                 scheduleMuniRemoteTabClose('completed_with_error');}}
